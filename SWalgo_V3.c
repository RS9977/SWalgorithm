...
int main(int argc, char* argv[]) {
    ...
    for (i = 2; i < m+n-1; i++) { //Lines
        ...
        __m512i* Hu = (__m512i*) H+ind_u;
        __m512i* Hl = (__m512i*) H+ind_l;
        __m512i* Hd = (__m512i*) H+ind_d;
        __m512i* HH = (__m512i*) H+ind;
        __m512i* PP = (__m512i*) P+ind;

        //int Vsize = 512/sizeof(typeof(H));
      //  #pragma gcc ivdep 
        for (j = j_start; j <j_end-Vsize; j+=Vsize) { //Columns  

            __m512i Joffset = _mm512_add_epi32(offset, _mm512_set1_epi32(j));
            __m512i Ioffset = _mm512_set1_epi32(i);
            __mmask16 mask  = _mm512_cmpgt_epi32_mask(_mm512_set1_epi32(m), Ioffset);
            ...
            __m512i I_MJ1   = _mm512_sub_epi32(I_J,_mm512_set1_epi32(m-1));
...
void similarityScoreIntrinsic(__m512i* HH, __m512i* Hu, __m512i* Hd, __m512i* Hl, __m512i* PP, __m512i ii, __m512i jj, int* H, long long int ind, long long int max_len, long long int* maxPos, long long int *maxPos_max_len) {
    ...
    //Get element above
    up                     = _mm512_add_epi32(*Hu, _mm512_set1_epi32(gapScore));

    //Get element on the left
    left                   = _mm512_add_epi32(*Hl, _mm512_set1_epi32(gapScore));
    ...
    __m512i MATCHMISS      = _mm512_mask_mov_epi32(MATCHSCORE, mask, MISSMATCHSCORE);
    diag                   = _mm512_add_epi32(*Hd, MATCHMISS);
